...
final class ObjectManager {
	...
	public final void method180(byte data[], int regionY, int regionX, int regionChunkX, int regionChunkY, CollisionMap aclass11[]) {
		...
		for (int plane = 0; plane < 4; plane++) {
			for (int x = 0; x < 64; x++) {
				for (int y = 0; y < 64; y++)
					method181(y + regionY, regionChunkY, stream, x + regionX, plane, 0, regionChunkX);
	...
	private void method181(int mapY, int regionChunkY, Buffer stream, int mapX, int height, int i1, int regionChunkX) {
		try {
			if (mapX >= 0 && mapX < 104 && mapY >= 0 && mapY < 104) {
				mapTerrainAttributes[height][mapX][mapY] = 0;
				do {
					int attributeId = stream.readUnsignedByte();

					if (attributeId == 0) {
						if (height == 0) {
							anIntArrayArrayArray129[0][mapX][mapY] = -method172(0xe3b7b + mapX + regionChunkX, 0x87CCE + mapY + regionChunkY) * 8;
							return;
						} else {
		...
		label0:
		{
			...
			do {
				...
				do {
					...
					int more = stream.readUnsignedByte();
					int type = more >> 2;
					int rotation = more & 3;
		...
		for (; ; ) {
			...
			for (; ; ) {
				if (bool_255_) {
					...
					int plane = stream.readUnsignedByte() >> 2;
					int mapX = localX + x;
					int mapY = localY + y;
