...
public class Model extends Renderable {
	...
	public Model(int modelId) {
		byte[] is = modelHeaders[modelId].bytes;
	...
	public void decodeNewModel(byte data[], int modelId) {
		...
		anInt1626 = nc1.readUnsignedShort();
		anInt1630 = nc1.readUnsignedShort();
		...
		for (face = 0; face < anInt1630; face++) {
			...
			if (texture_opcode == 1) {
				texture[face] = (short) (nc6.readUnsignedShort() - 1);
				if (texture[face] >= 0) {
	...
	private void decodeOldModel(byte[] data, int modelId) {
		...
		anInt1626 = stream.readUnsignedShort();
		anInt1630 = stream.readUnsignedShort();
	...
	public static void method460(byte abyte0[], int j) {
		try {
			...
			ModelHeader header = modelHeaders[j] = new ModelHeader();
			header.bytes = abyte0;
			header.anInt369 = buffer.readUnsignedShort();
			header.anInt370 = buffer.readUnsignedShort();
			header.anInt371 = buffer.readUnsignedByte();
	...
	public static Model getModel(int modelId) {
		...
		if (header == null) {
			onDemandFetcher.provide(modelId);
			return null;
		}
	...
	static Provider onDemandFetcher;
